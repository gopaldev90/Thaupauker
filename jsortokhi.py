...
class RTONDecoder():
	...
	# function that converts rton to json
	def tojson(self, inp):
		try:
			file = open(inp, "rb")
			HEADER = file.read(2)
			if not HEADER == b"\x10\0":
				HEADER += file.read(2)
				if HEADER == b"RTON":
					data = self.parse_root_object(file)
					return data
...
class JSONDecoder():
	...
	# type 83
		if "@" in string:
			...
			return b"\x84"
	# function that converts json to rton
	def torton(self, file):
		return (self.encode_root_object(file))
...
def tojson(rtonpath):
    ...
    json_data = enc.tojson(rtonpath)
    print('to json converted successfully')
    return json_data
...
def process_files_in_place(work_dir, convert_func, ext_map,mita=True):
    for dirpath, dirnames, filenames in os.walk(work_dir):
        for filename in filenames:
            ...
            banane= os.path.splitext(filename)[0]
            dest_ext = ext_map.get(src_ext)
            if dest_ext:
                if banane=='pp':
                	if dest_ext =='.rton':
                		dest_ext ='.dat'
                dest_filename = filename.replace(src_ext, dest_ext)
                dest_path = os.path.join(dirpath, dest_filename)
                if mita:
                	odf=os.path.join(dirpath,filename)
                else:
                	odf=None
                try:
                    data = convert_func(src_path)
                    save_data(data, dest_path,odf)
                except Exception as e:
                    print(f"Error processing {filename}: {e}")

if __name__ == "__main__":
    print("1 for JSON to RTON")
    print("2 for RTON to JSON")
    mode = input().lower()
    l= ['r','j']
    if('.' in mode):
    	mita=False
    ...
    if mode in l:
    	...
    	mode=int(mode)
    if mode<=2:
	    workdir = '/storage/emulated/0/Documents/Work'
	    if mode == 1:
	        ext_map = {'.json': '.rton'}
	        func=torton
	    elif mode == 2:
	        ...
	        ext_map['.dat']='.json'
	        func=tojson
	    process_files_in_place(workdir, func, ext_map, mita)
